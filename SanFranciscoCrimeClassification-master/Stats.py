...
features = ['X', 'Y', 'Intersection', 'Year', 'Month', 'Day', 'Hour', 'Minute', 'DayNumber', 'DistrictNumber']
...
pvalues = []
for i in features:
	z_stat, p_val = stats.ranksums(fraud[i], training_data[i])
	pvalues.append(p_val)

#scores = zip(pvalues, features)
#scores.sort(key=pvalues)
pvalues, features = zip(*sorted(zip(pvalues, features), reverse=False, key=lambda x: x[0]))
